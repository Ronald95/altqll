...
// PÁGINAS (Tus importaciones se mantienen igual...)
...
import Posiciones from "./pages/Naves/posat/index";
import Wind from "./pages/Naves/posat/WindyFleetMap";
import SueldosIndex from "./pages/Sueldos/Index";
import ReporteMapa from "./pages/Naves/posat/reporteMapa";
...
const PublicRoute = ({ children }) => {
  ...
  // Mientras el AuthContext está inicializando, mostramos el spinner
  if (loading) return <GlobalLoadingSpinner />;
  
  // Si ya está autenticado, no lo dejamos entrar a login/signup
...
// ============================================
// APP CONTENT (Lógica de carga global)
// ============================================
const AppContent = () => {
  const { loading } = useAuth();

  // BLOQUEO INICIAL: Mientras initializeAuth() esté en curso,
  // evitamos renderizar el Router para que no haya parpadeos.
  if (loading) {
      ...
      <Routes>
        ...
        <Route element={<ProtectedRoute fallback={<GlobalLoadingSpinner />} />}>
          <Route element={<AppLayout />}>
            ...
            {/* PDF Scanner */}
            <Route element={<ProtectedRoute permissions="aplication.add_processedpdf" />}>
            ...
            {/* Sueldos */}
            <Route element={<ProtectedRoute permissions="aplication.add_processedpdf" />}>
              <Route path="/comparar-sueldos" element={<SueldosIndex />} />
            </Route>
            
            {/* Trabajadores */}
            <Route element={<ProtectedRoute permissions="aplication.view_trabajador" />}>
              <Route path="/trabajadores" element={<Trabajador_Index />} />
            </Route>
            {/* Posiciones */}
            <Route element={<ProtectedRoute permissions="aplication.view_posicion" />}>
              <Route path="/posat-naves" element={<Posiciones />} />
              <Route path="/posat-naves-wind" element={<Wind />} />
              <Route path="/posat-naves-map" element={<ReporteMapa />} />
            </Route>

            {/* Naves */}
            <Route element={<ProtectedRoute permissions="aplication.view_naves" />}>
...
// ============================================
// APP PRINCIPAL (Solo el Provider)
// ============================================
